...
class RegularMessagesController extends Controller
{
    /*
    status codes:
        1 -> active
    ...
    */
    public function list(Request $request, $page=1)
    {
        ...
        $messages = DB::table('messages')
            ->limit($size)
            ->offset(($page-1)*$size)
            ->whereRaw('messages.sender = ' . Auth::user()->id . ' OR messages.reciever = ' . Auth::user()->id);
        ...
        return view('regular.messages.list', [
            'messages'      => $messages,
            'page'          => $page,
            'pageSize'      => $size,
            'pageCount'     => ceil($messagesCount / $size),
    ...
    public function view(Request $request, $id){
        ...
        return view('regular.messages.view', [
            'message'        => $message,
        ]);
    }
    ...
    public function newGet(Request $request, $recieverId=0){
        if($recieverId != 0){
            ...
            return view('regular.messages.new',[
            ]);
        }
    }
        ...
        if($validator->fails()){
            ...
            foreach($oldInputs['recievers'] as $reciever){
                DB::table('messages')->insert([
                    ...
                    'reciever_name' => $reciever->title,
                    'status'        => 1

                ]);
            }
            return redirect()->intended('user/messages');
        }
    }
