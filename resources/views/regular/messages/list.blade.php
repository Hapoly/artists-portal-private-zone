    ...
    <div class="col s12 m8 offset-m2">
        <nav>
            <div class="nav-wrapper teal">
                <form method="get" action="{{url('user/messages/' . $sort)}}">
        ...
        <div class="card-panel white">
            @if (sizeof($messages) > 0)
                ...
                <ul class="pagination center">
                    <li class="{{$page==$pageCount?'disabled':'waves-effect'}}"><a href="{{url('user/messages/' . ($page+1) . $sort)}}"><i class="material-icons">chevron_right</i></a></li>
                    <li class="active"><a href="#!">{{$page.'/'.$pageCount}}</a></li>
                    <li class="{{$page==1?'disabled':'waves-effect'}}"><a href="{{url('user/messages/' . ($page-1) . $sort)}}"><i class="material-icons">chevron_left</i></a></li>
                </ul>
            ...
            @endif
            <a href="{{url('user/message-new/')}}" class="waves-effect waves-light btn right" target="_blank">پیام جدید</a>
        </div>
    </div>
</div>
